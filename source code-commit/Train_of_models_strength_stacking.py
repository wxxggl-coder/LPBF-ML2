...
# 计算训练集的准确度（R2分数）和均方根误差
accuracy = r2_score(y_train, y_pred_train)
sqrterror1 = math.sqrt(mean_squared_error(y_train, y_pred_train))
S_accuracy = 1 - np.mean(np.abs(y_pred_train - y_train) / y_train)
print("训练集平均强度R2：", accuracy)
print("训练集平均强度的均方根误差：", sqrterror1)
print('训练集平均强度的预测准确度：', S_accuracy)


# print("正规方程的均方误差为：\n",error)
sqrterror = math.sqrt(error)
print("stacking的均方根误差为：\n",sqrterror)
percenterror = sqrterror / np.mean(np.abs(y_test))
print("stacking预测的误差为：\n",percenterror)
#r2
r2 = r2_score(y_test, y_pred_test)
print('stacking的r2为：', r2)
S_accuracy = 1 - np.mean(np.abs(y_pred_test - y_test) / np.abs(y_test))
print('stacking预测准确度：', S_accuracy)
#
# # 创建一个新的工作簿和工作表
...
# sheet.cell(row=1, column=5).value = "真实强度(MPa)"
# sheet.cell(row=1, column=6).value = "预测强度(MPa)"
#
# # 写入数据
...
#
# # 保存为.xlsx文件
# wb.save('F:/Ti、B有能量密度/prediction_stacking_11_train_strength.xlsx')
#
# # 创建一个新的工作簿和工作表
# wb = openpyxl.Workbook()
# sheet = wb.active
...
#
# # 保存为.xlsx文件
# wb.save('F:/Ti、B有能量密度/prediction_stacking_11_test_strength.xlsx')