...
# 将数据分为训练集和测试集
x_train_true, x_test_true, y_train, y_test = train_test_split(X_true, y, test_size=0.2, random_state=42)
#训练集中对应的x真实值
...
print("强度R2：", accuracy)
print("强度的均方根误差：", sqrterror1)
print('强度的预测准确度：', S_accuracy)
...
# 保存为.xlsx文件
wb.save('F:/Ti、B有能量密度/prediction_LGBM_11_train_strength.xlsx')
...
# 保存为.xlsx文件
wb.save('F:/Ti、B有能量密度/prediction_LGBM_11_test_strength.xlsx')